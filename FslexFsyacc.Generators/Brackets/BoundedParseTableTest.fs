...
type BoundedParseTableTest(output: ITestOutputHelper) =
    let moduleName = "BoundedParseTable"
    let moduleFullName = $"FslexFsyacc.Brackets.{moduleName}"
    let moduleFilePath = Path.Combine(Dir.bootstrap, $"Brackets/{moduleName}.fs")

    let filePath = Path.Combine(__SOURCE_DIRECTORY__, @"bounded.fsyacc")
    ...
    member _.``06 - generate ParseTable``() =
        let outp = coder.generateModule(moduleFullName)
        File.WriteAllText(moduleFilePath, outp, Encoding.UTF8)
        output.WriteLine("output yacc:")
        output.WriteLine(moduleFilePath)
    ...
    member _.``07 - valid ParseTable``() =
        ...
        let header,semans =
            let text = File.ReadAllText(moduleFilePath, Encoding.UTF8)

...
type FsyaccFile = 
    ...
    member this.toFsyaccParseTable2() = 
        let mainRules: (string list * string) list = this.mainRules |> List.map Triple.ends
        ...
        let isemantics = 
            let mp =  // prod -> semantic
                mainRules
                |> List.map(fun(ls,s)->Array.ofList ls,s)
                |> Map.ofList 
            productions
            |> Array.filter (fun (i, prod) -> i < 0 && prod.[0] > "")
            |> Array.map(fun (i, prod) -> i, mp.[prod])

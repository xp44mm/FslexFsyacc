...
module DFATools =
    /// DFA nextState(s,a)
    let getNextStates (transitions:Set<uint32*'tag*uint32>):Map<uint32,Map<'tag,uint32>> = 
        Set.toUniqueJaggedMap transitions

    //鍵：單個的接受狀態。-> 值：接受状态键对应的lexeme State集合。
    let getLexemesFromFinal(finalLexemes:list<Set<uint32>*Set<uint32>>):Map<uint32,Set<uint32>> =
        finalLexemes

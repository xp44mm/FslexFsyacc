...
- `joinOp` and `onWord` represent keywords for join-like operations that are declared in `CustomOperationAttribute`. For example, 

  ```fsharp
  [<CustomOperator("join", IsLikeJoin=true, JoinConditionWord="on")>]
  ```

   declares “join” and “on”.

- Similarly, `groupJoinOp` represents a keyword for groupJoin-like operations, declared in `CustomOperationAttribute`. For example, 

  ```fsharp
  [<CustomOperator("groupJoin", IsLikeGroupJoin=true, JoinConditionWord="on")>]
  ```

   declares “groupJoin” and “on”.
...
`e_(1)`: the computation expression being translated
...
type SimpleSequenceBuilder() = 
    member _.For (source : seq<'a>, body : 'a -> seq<'b>) = 
           seq { for v in source do yield! body v } 
    member _.Yield (item:'a) : seq<'a> = seq { yield item } 
...
type SimpleSequenceBuilder() = 
    ...
    member _.Where (source : seq<'a>, f: 'a -> bool) : seq<'a> = Seq.filter f source        
...
type SimpleSequenceBuilder() = 
    ...
    member _.Yield (item:'a) : seq<'a> = seq { yield item } 
    [<CustomOperation("where")>] 
    member _.Where (source: seq<'a>, [<ProjectionParameter>]f: 'a -> bool) : seq<'a> =
           Seq.filter f source
...
`ProjectionParameterAttribute` is useful when a let binding appears between `for` and the custom operators. For example, the expression
...
Now, assume that we want to write the condition “
...
type SimpleSequenceBuilder() = 
    ...
    member _.Merge (src1:seq<'a>, src2:seq<'a>, ks1, ks2, ret) = 
...
- If the delegate type has an Invoke method that has the following signature Invoke(*ty₁*,...,*ty_(n)*) -> *rty_(A)*,
...
- A array lookup *expr*_(1a).[*expr*_(1b)] where *expr*_(1a) has type *ty*[].
  ...
  Type checking of *expr₂* uses the expected result type *ty* and generates the an elaborated expression *expr₂*. The overall elaborated expression is an assignment to a field (see §6.9.4):
...
If the assertion fails, the type *ty(expr)* may also be of any static type that satisfies the “collection pattern” of CLI libraries. If so, the *enumerable extraction* process is used to enumerate the type. In particular, *ty_(expr)* may be any type that has an accessible `GetEnumerator` method that accepts zero arguments and returns a value that has accessible `MoveNext` and `Current` properties. The type of *pat* is the same as the return type of the `Current` property on the enumerator value. However, if the `Current` property has return type obj and the collection type *ty* has an Item property with a more specific (non-object) return type *ty₂*, type *ty₂* is used instead, and a dynamic cast is inserted to convert `v.Current` to *ty₂*.
...
```fsharp
let *function-or-value-defns*     in *expr*
...
```fsharp
mutable_opt *access_opt* *pat* *typar-defns_opt* *return-type_opt* = *rhs-expr
```
...
```fsharp
inline*_opt* *access_opt* 
*ident-or-op* *typar-defns_opt* *pat₁* ... *pat_(n)* *return-type_opt* = 
*rhs-expr*
```
...
The expression is first checked as an expression of form `let *ident* = *expr₁* in *expr₂*` , which results in an elaborated expression of the following form:
...
The initial type of the overall expression is *ty*. Expression *expr* is checked using a fresh initial type *ty_(e)*, with constraint *ty_(e) :> ty*. 
...
In F#, the warning “copy due to possible mutation of value type” is a level 4 warning and is not reported when using the default settings of the F# compiler. This is because the majority of value types in CLI libraries are immutable. This is warning number 52 in the F# implementation.
...
```fsharp
*rule* :=
    *pat* *pattern-guard_opt* -> *expr* -- pattern, optional guard and action

*pattern-guard* := when *expr*

*pat* :=
    *const* -- constant pattern
    *long-ident* *pat-param_opt* *pat_opt* -- named pattern
    _ -- wildcard pattern
...
The pattern [*pat₁* ; ... ; *pat_(n)*] is shorthand for a series of :: and empty list patterns *pat₁* :: … :: *pat_(n)* :: [].
...
*type-name* :=
    *attributes_opt access_opt *ident* *typar-defns*_opt

*abbrev-type-defn* :=
    *type-name* = *type*

*union-type-defn* :=
    *type-name* '=' *union-type-cases* *type-extension-elements*_opt
...
*record-type-defn* :=
    *type-name* = '{' *record-fields* '}' *type-extension-elements_opt*

*record-fields* :=
    *record-field* ; ... ; *record-field* ;*_opt*

*record-field* :=
    *attributes_opt* mutable*_opt* *access_opt* *ident* : *type*
...
*class-type-body* :=
    *class-inherits-decl_opt class-function-or-value-defns_opt type-defn-elements_opt*
...
*exception-defn* :=
*attributes_opt* exception *union-type-case-data* -- exception definition
*attributes_opt* exception *ident* = *long-ident* -- exception abbreviation

*enum-type-defn* :=
*type-name '*=' *enum-type-cases*

*enum-type-cases* =
'|'*_opt* *enum-type-case* '|' ... '|' *enum-type-case*
...
*primary-constr-args* :=
*attributes_opt* *access_opt* ( *simple-pat, ... , simple-pat* )
...
*member-defn* :=
*attributes_opt* static*_opt* member *access_opt* *method-or-prop-defn* -- concrete member
*attributes_opt* abstract member*_opt* *access_opt* *member-sig* -- abstract member
*attributes_opt* override *access_opt* *method-or-prop-defn* -- override member
*attributes_opt* default *access_opt* *method-or-prop-defn* -- override member
*attributes_opt* static*_opt* val mutable*_opt* *access_opt* *ident* *: type* -- value member
...
*ident._opt* *ident* with *function-or-value-defns* -- property definition via get/set methods

member *ident* = *expr* -- auto-implemented property definition

member *ident* = *expr* with get -- auto-implemented property definition

member *ident* = *expr* with set -- auto-implemented property definition

member *ident* = *expr* with get,set -- auto-implemented property definition

member *ident* = *expr* with set,get -- auto-implemented property definition

*member-sig* :=
*ident* *typar-defns_opt* : *curried-sig* -- method or property signature
*ident* *typar-defns_opt* : *curried-sig* with get -- property signature
*ident* *typar-defns_opt* : *curried-sig* with set -- property signature
*ident* *typar-defns_opt* : *curried-sig* with get,set -- property signature
*ident* *typar-defns_opt* : *curried-sig* with set,get -- property signature
...
*arg-spec* :=
     *attributes_opt* *arg-name-spec_opt* *type*
...
```fsharp
type *type-name* *pat_opt* *as-defn_opt* =
...
```fsharp
static*_opt* member *ident*.*_opt* *ident* = *expr*
static*_opt* member *ident*.*_opt* *ident* with get *pat* = *expr*
static*_opt* member *ident*.*_opt* *ident* with set *pat_opt pat*= *expr*
static*_opt* member *ident*.*_opt* *ident* with get *pat* = *expr* and set *pat_opt pat* = *expr*
static*_opt* member *ident*.*_opt* *ident* with set *pat_opt pat* = *expr* and get *pat* = *expr
```

A property member in the form

```fsharp
static*_opt* member *ident*.*_opt* *ident* with get *pat₁* = *expr₁* and set *pat_(2a) pat_(2b)*_opt = *expr₂*
```
...
```fsharp
static*_opt* member *ident*.*_opt* *ident* with get *pat₁* = *expr₁*
static*_opt* member *ident*.*_opt* *ident* with set *pat_(2a) pat_(2b)* _opt = *expr₂
```
...
```fsharp
static*_opt* member *ident*.*_opt* *ident* = *expr*
static*_opt* member *ident*.*_opt* *ident* with get () = *expr*
```

These two are also equivalent:

```fsharp
static*_opt* member *ident*.*_opt* *ident* with set *pat* = *expr₂*
static*_opt* member *ident*.*_opt* *ident* with set () *pat* = *expr*
```
...
```fsharp
static*_opt* member *ident*.*_opt* *ident* with get *pat_(idx)* = *expr*

static*_opt* member *ident*.*_opt* *ident* with set *pat_(idx) pat* = *expr*
```

The *ident*.*_opt* must be present if and only if the property member is an instance member. When evaluated, the identifier *ident* is bound to the “this” or “self” object parameter that is associated with the object within the expression *expr*.
...
As of F# 3.1, the special .[] notation for Item properties is available only for instance members. A static indexer property cannot be accessible by using the .[] notation.
...
```fsharp
static*_opt* member val *access_opt* *ident* *:* *ty_opt* = expr
static*_opt* member val *access_opt* *ident* *:* *ty_opt* = expr with get
static*_opt* member val *access_opt* *ident* *:* *ty_opt* = expr with set
static*_opt* member val *access_opt* *ident* *:* *ty_opt* = expr with get, set
```
...
```fsharp
static*_opt* member *ident*.*_opt* *ident* *pat₁* ... *pat_(n)* = *expr*
```

The *ident*.*_opt* can be present if and only if the property member is an instance member. In this case, the identifier *ident* corresponds to the “this” (or “self”) variable associated with the object on which the member is being invoked.
...
*measure-literal-atom* *measure-literal-atom* ^ *int32* -- power of measure, such as m^3
...
*attributes_opt* module *access_opt* *ident* = *module-defn-body*

*module-defn-body* :=

begin *module-elems_opt* end
...
```
let rec*_opt* *function-or-value-defn₁* and *...* and *function-or-value-defn_(n)*
```
...
val mutable*_opt* *curried-sig* -- value signature
...
*attributes_opt* access*_opt* new : *uncurried-sig* -- constructor signature

*attributes_opt* member access*_opt* *member-sig* -- member signature

*attributes_opt* abstract access*_opt* *member-sig* -- member signature

*attributes_opt* override *member-sig* -- member signature

*attributes_opt* default *member-sig* -- member signature

*attributes_opt* static member access*_opt* *member-sig* -- static member signature

interface *type* -- interface signature

*abbrev-type-signature* := *type-name* '=' *type*

*union-type-signature* := *type-name* '=' *union-type-cases type-extension-elements-signature_opt*

*record-type-signature* := *type-name* '=' '{' *record-fields* '}' *type-extension-elements-signature_opt*
...
1.  If a corresponding signature *Sig_(file)* exists, check *Impl_(file)* against *Sig_(file)* during this process (§11.2). Then add *Sig_(file)* to *impl-env_(i-1)* to produce *impl-env(i)*. This step makes the signature-constrained view of the implementation file available for use in later implementation files. The processing of the implementation file has no effect on the signature environment, so *sig-env_(i)* is identical to *sig-env_(i-1)*.

&nbsp;

3.  If the implementation file has no signature file, add *Impl_(file)* to both *sig-env_(i-1)* and *impl-env_(i-1)*, to produce *sig-env(i)* and *impl-env*_(i). This makes the contents of the implementation available for use in both later signature and implementation files.
...
3.  For each such *F*, attempt to resolve *ident*[.*rest*] in the following order. If any resolution succeeds, then terminate the search:
   ...
   4)  A value in *F*. Return this item and *rest*.

   5)  A union case in *F*. Return this item and *rest*.

   6)  An exception constructor in *F*. Return this item and *rest*.

   7)  A type in *F*. If *rest* is empty, then return this type; if not, resolve using *Name Resolution for Members*.

   8)  A [sub-]module in *F*. Recursively resolve *rest* against the contents of this module.
...
3. If steps 1 and 2 do not resolve *long-ident*, look up *ident* in the *Types* table.
   ...
   2.  If no generic arity *n* is available, and *rest* is not empty:
      ...
      4)  If the *Types* table contains a type *ident* that does not have generic arguments, resolve to this type.

      5)  If the *Types* table contains a unique type *ident* that has generic arguments, resolve to this type. However, if the overall result of the *Name Resolution in Expressions* operation is a member, and the generic arguments do not appear in either the return or argument types of the item, warn that the generic arguments cannot be inferred from the type of the item.

      6)  If neither of the preceding steps resolves the type, give an error.

7.  If *rest* is empty, return the type, otherwise resolve using *Name Resolution for Members*.
...
2. At each type, try to resolve *member-ident* to one of the following, in order:
   ...
   3.  A property group of *type*.

   4.  A method group of *type*.

   5.  A field of *type*.

   6.  An event of *type*.

   7.  A property group of extension members of *type*, by consulting the *ExtensionsInScope* table.

   8.  A method group of extension members of *type*, by consulting the *ExtensionsInScope* table.

   9.  A nested type *type-nested* of *type*. Recursively resolve .*rest* if it is present, otherwise return *type-nested*.

10. At any type, the existence of a property, event, field, or union case named *member-ident* causes any methods or other entities of that same name from base types to be hidden.

11. Combine method groups with method groups from base types. For example:
...
3.  Divide *long-ident* into [*namespace-or-module-path*].*ident*[.*rest*], in which the *namespace-or-module-path* becomes successively longer.

4.  For each such division, consider each module and namespace declaration group *F* in the list that results from resolving *namespace-or-module-path* by using *Name Resolution in Module and Namespace Paths* (§14.1.2).

5.  For each such *F*, attempt to resolve *ident*[.*rest*] in the following order. Terminate the search when the expression is successfully resolved.
...
2.  If the method group contains more than one method, the expected overall type of the expression is asserted to be a function type *dty* -> *rty*.
...
2.  Determine the *named* and *unnamed formal parameters*, called *NamedFormalArgs* and *UnnamedFormalArgs* respectively, by splitting the formal parameters for M into parameters that have a matching argument in *NamedActualArgs* and parameters that do not.

3.  If the number of *UnnamedFormalArgs* exceeds the number of *UnnamedActualArgs*, then modify *UnnamedFormalArgs* as follows:
...
1.  Associate each *name* = *arg* in *NamedActualArgs* with a *target*. A target is a *named formal parameter*, a *settable return property*, or a *settable return field* as follows:
...
2.  Verify that the type of *obj* is a subtype of the containing type of the method M.

3.  For each *UnnamedActualArg_(i)* and *UnnamedFormalArg_(i)*, verify that the corresponding *ActualArgType* coerces to the type of the corresponding argument of M.

4.  If M_(possible) uses ParamArray conversion with type *pty*, then for each *ParamArrayActualArg_(i)*, verify that the corresponding *ActualArgType* coerces to *pty*.

5.  For each *NamedActualArg_(i)* that has an associated formal parameter target, verify that the corresponding *ActualArgType* coerces to the type of the corresponding argument of M.

6.  For each *NamedActualArg_(i)* that has an associated property or field setter target, verify that the corresponding *ActualArgType* coerces to the type of the property or field.

7.  Verify that the *prospective formal return type* coerces to the *expected actual return type*. If the method M has return type *rty*, the *formal return type* is defined as follows:
...
```
(static_opt member *ident* : *arg-type*₁ * ... * *arg-type*_(n) *-> ret-type*)  
```
...
- *type* is a named type whose type definition contains the following member, which takes *n* arguments:
  ...
  static_opt member *ident* : *formal-arg-type*₁ * ... * *formal-arg-type*_(n) *-> ret-type*
...
```
inline*_opt* *ident₁ pat₁* ... *pat_(n)* :*_opt* *return-type_opt* = *rhs-expr*
```
...
```
mutable*_opt* *pat* :*_opt* *type_opt* = *rhs-expr*
```
...
```
static*_opt* member *ident_opt* *ident* *pat₁* ... *pat_(n)* = *expr*
```
...
See also the online documentation at [here](http://msdn.microsoft.com/library/ee353567.aspx).
...
Note that the for-expression form 

```fsharp
for *var* = *expr*₁ downto *expr*₂ do *expr*₃ 
```

is also permitted for ML compatibility.
...
*module-defn* : 
*attributes_opt* module *access_opt* *ident* = **begin_opt** *module-defn-body* **end_(op)**
